...
public:
    ...
    static bool HandleBetterReloadItemsCommand(ChatHandler* handler, const char* args)
    {
        ...
        for (auto const& itr : Acore::Tokenize(args, ' ', false))
        {
            ...
            itemTemplate->Armor = uint32(fields[55].Get<uint16>());
            itemTemplate->HolyRes = uint32(fields[56].Get<uint8>());
            itemTemplate->FireRes = uint32(fields[57].Get<uint8>());
            itemTemplate->NatureRes = uint32(fields[58].Get<uint8>());
            itemTemplate->FrostRes = uint32(fields[59].Get<uint8>());
            itemTemplate->ShadowRes = uint32(fields[60].Get<uint8>());
            itemTemplate->ArcaneRes = uint32(fields[61].Get<uint8>());
